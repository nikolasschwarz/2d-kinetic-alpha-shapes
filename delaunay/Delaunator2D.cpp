...
inline bool orient(const float px, const float py, const float qx, const float qy, const float rx, const float ry)
{
...
inline bool in_circle(const float ax, const float ay, const float bx, const float by, const float cx, const float cy,
  ...
  , m_edge_stack()
{
  ...
  for (std::size_t i = 0; i < n; i++)
  {
    ...
    const float r = circumradius(i0x, i0y, i1x, i1y, coords[2 * i], coords[2 * i + 1]);
