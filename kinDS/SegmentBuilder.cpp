...
void SegmentBuilder::afterEvent(KineticDelaunay::Event& e)
{
  ...
  for (size_t he_id : { he0_id, he1_id, he2_id, he3_id })
  {
    ...
    // TODO: I'm not sure how to determine whether we have to update the left or right vertex here
...
void SegmentBuilder::finalize(double t)
{
  ...
  for (size_t i = 0; i < half_edge_count; i += 2)
  {
    ...
    size_t left_index = mesh.addVertex(left_vertex[0], left_vertex[1], left_vertex[2]);
    size_t right_index = mesh.addVertex(right_vertex[0], right_vertex[1], right_vertex[2]);

    // add last vertex indices
    segment_mesh_pair_last_left_and_right_vertex.push_back(
      std::make_pair(left_index, right_index));
  }
