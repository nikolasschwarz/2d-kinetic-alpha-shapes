...
StrandTree::StrandTree(const std::vector<std::vector<glm::dvec2>>& support_points,
  ...
  void writeVec2(std::ostream& out, const glm::dvec2& v)
  {
    out << v.x << ' ' << v.y << '\n';
  }

  void readVec2(std::istream& in, glm::dvec2& v)
  {
    in >> v.x >> v.y;
  }
  ...
  void StrandTree::saveToFile(const std::string& path) const
  {
    std::ofstream out(path);
    if (!out)
      throw std::runtime_error("StrandTree::saveToFile: cannot open " + path + " for writing");
    out << FORMAT_MAGIC << '\n';
    out << height << '\n';
  ...
  StrandTree StrandTree::loadFromFile(const std::string& path)
  {
    std::ifstream in(path);
    if (!in)
      throw std::runtime_error("StrandTree::loadFromFile: cannot open " + path + " for reading");
    std::string line;
    auto getLine = [&in, &line]() -> bool {
